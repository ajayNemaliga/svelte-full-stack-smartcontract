...
>
  ...
  {#if form?.deploy}
    ...
    <p class="pt-2">Bounty was deployed successfully <a href="https://whatsonchain.com/tx/{form?.txid}">Trancation Id</a></p>
  {/if}
</div>
<div
  class="mt-10 pt-10 mb-20 w-full max-w-xl p-12 mx-auto rounded-lg shadow-xl dark:bg-white/10 bg-white/30 ring-1 ring-gray-900/5 backdrop-blur-lg"
>
  ...
  {#if form?.unlock}
    <!-- this message is ephemeral; it exists because the page was rendered in
		   response to a form submission. it will vanish if the user reloads -->
    <p class="pt-2">Bounty was unlocked successfully <a href="https://whatsonchain.com/tx/{form?.txid}">Trancation Id</a></p>
  {/if}
</div>